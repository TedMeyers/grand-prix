...
public class TableSorter extends AbstractTableModel {
	static final long serialVersionUID = 123006;
	...
	public int modelIndex(int viewIndex) {
		...
		rows = getViewToModel();
		if (viewIndex < rows.length) {
			System.out.println("Good: " + viewIndex + "/" + rows.length + ": " + tableModel.getRowCount());
			return rows[viewIndex].modelIndex;
		}
		System.out.println("Warning!!, table sorter model index not found: " + viewIndex + "/" + rows.length + ": " + tableModel.getRowCount());
		return rows[rows.length-1].modelIndex;
	}
